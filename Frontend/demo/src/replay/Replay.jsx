...
function Replay(props) {
  ...
  const [all_user, setAll_user] = useState(
    [
      { id: '0', name: 'No', lastname: 'User', difficulty: '' },
      { id: '1', name: 'Jane', lastname: 'Doe', difficulty: 'hard' },
      { id: '2', name: 'Max', lastname: 'Mustermann', difficulty: 'medium' },
      { id: '3', name: 'Maria', lastname: 'Mustermann', difficulty: 'easy' },
    ]
  );
  ...
  const [current_activity, setCurrent_activity] = useState('Activity_0dwnqmx')
  // const [current_activity, setCurrent_activity] = useState('Activity_0lasejm')
  ...
  const [current_Button, setCurrent_Button] = useState('')
  ...
  // const [grünerSchrauber, setGrünerSchrauber] = useState(false)  
  ...
  const [instance, setInstance] = useState('2023-04-21T09:24:36.072Z');
  const [allInstance, setAllInstance] = useState(
    [
      // { id: '0', instanceId: '', name: 'Bitte Instanz auswählen'},
      // { id: '1', instanceId: '61c69be2-1cc0-4c91-ae7c-e449a46f1575', name: 'Instanz 1'},
      // { id: '2', instanceId: '85ac81ad-bb3d-4e55-b3b5-b01c4d35dfc6', name: 'Instanz 2'},
      // { id: '3', instanceId: 'd32ff98d-f671-49df-a68e-784c02ee8e49', name: 'Instanz 3'},
      // { id: '4', instanceId: '29d9cbd9-678e-4972-a59d-75804a47288f', name: 'Instanz 4'},
      // { id: '5', instanceId: '1a2ad18f-d19b-469a-b0f0-00cf2d8893c1', name: 'Video Aufbau'},
      // { id: '6', instanceId: '6206f3e8-04f8-47dc-ad09-c9ef7dbc551a', name: 'Instanz 6'},
    ]
  )
  const [progress, setProgress] = useState(0.35)

  const [iterationId, setIterationId] = useState("bd299460-f907-4647-9374-68a8e1d13ef3")
  ...
  useEffect(() => {
    ...
    if (location.state != null) {
      setInstance(location.state['RunName'])
    }
    axios.post('/statistics', {
      ...
      .then(function (response) {
        ...
        for (const [key, value] of Object.entries(response.data['durations_per_run'])) {
          setAllInstance((instances) => [...instances, key])
        }
  ...
  const multifunction = (f) => {
    ...
    if (f === 'resume') {
      console.log('resume')
      startLoop()
      setCurrent_Button('Resume')
      axios.post('/continue_replay_page', {
      })
        .then(function (response) {
          console.log(response.data)
        })
    ...
    loop_id =
      setInterval(() => {
        ...
        axios.get('/status').then(data => {
          ...
          console.log(data.data['nodes'][0]['name'])
          ...
          if (data.data['nodes'][0]['name'] !== 'unknown') {
            ...
            console.log(data.data['nodes'][0]['bpmn_obj']['id'] !== actual_activity)
  ...
  const handleNewActivity = (activityId) => {
    ...
    switch (activityId) {
      ...
      case "Activity_16qudgf":
        ...
        setCurrent_material('Lagerdeckel')
        setActivity_name('Lagerdeckel entnehmen')
        setKolbenstange(false)
        setBaugruppe(false)
        setMutter(false)
        setKolbenstangeBaugruppe(true)
  ...
  const handlenewInstance = (event) => {
    console.log(event.target.selectedIndex)
    setInstance(allInstance[event.target.selectedIndex])
    console.log(allInstance[event.target.selectedIndex])
    console.log(instance)
  ...
  return (
    <>
      ...
      <div className='factsStyle'>
        Avg Time:<br />
        {Math.floor(avgTimeHistory / 60)} min {Math.floor(avgTimeHistory % 60)} sek <br />
        Avg Score:<br />
        {Math.round(avgScoreHistory * 100 + Number.EPSILON) / 100} <br />
        Avg Failure:<br />
        {Math.round(avgFailureHistory * 100 + Number.EPSILON) / 100} <br />


      </div>
      <div className='instanzStyle'>
        <label>
          <select className='dropdown' value={instance} onChange={handlenewInstance}>
            {allInstance.map(i => (
      ...
      <div className='buttonstyle'>
        <div className='background-Playback' style={highlightButton('Playback')}></div>
        <div className='playback' onClick={() => multifunction('playback')}> <IoIcons.IoPlaySkipBack size={20} /> </div>
        <div className='background-Play' style={highlightButton('Play')}></div>
        <div className='play' onClick={() => multifunction('play')}> <IoIcons.IoPlay size={20} /> </div>
        <div className='background-Stop' style={highlightButton('Stop')}></div>
        <div className='stop' onClick={() => multifunction('stop')}> <IoIcons.IoPause size={20} /> </div>
        <div className='background-Resume' style={highlightButton('Resume')}></div>
        <div className='resume' onClick={() => multifunction('resume')}> <IoIcons.IoStop size={20} /> </div>
        <div className='background-Playforward' style={highlightButton('Playforward')}></div>
        <div className='playforward' onClick={() => multifunction('playforward')}> <IoIcons.IoPlaySkipForward size={20} /> </div>
      </div>

      <div className='replaybarStyle'>
        <div className='basicbar'></div>
        <div className='progressbar' style={colorProgressBar(progress)}></div>
      </div>
      ...
      <div className='werkBankStyle_replay'>
        {/* <WorkTableTest/> */}
        <WorkTable data={{
          mutter,
          kolbenstange,
      ...
      <div className='cylinderStyle_replay'>
